...
pub fn print(txt: &str) {
    for mut b in txt.bytes() {
        // update position every iteration so that if a print happens in main thread
        // and interrupt at the same time, things won't break
        let pos = CURSOR_POS.read();
        let (mut cursor_x, mut cursor_y) = (pos.x(), pos.y());
        ...
        CURSOR_POS.write(CursorPos::new().with_x(cursor_x).with_y(cursor_y));
    }
}
...
pub fn set_cursor_pos(x: u8, y: u8) {
    CURSOR_POS.write(CursorPos::new().with_x(x & 31).with_y(y & 31));
}
